...
public class AsyncFileProcessorService {
     ...
     *
     * <p>The task runs on a background thread. A spinner and toast are shown during processing.
     * On success, the result is passed to {@code onSuccess}. Errors are handled internally
     * (logged, toast shown, spinner hidden); no error callback is needed.
     *
    ...
    public <T> void processFileAsync(
        ...
        Task<Optional<T>> task = new Task<>() {
            ...
            protected Optional<T> call() {
                try {
                    ...
                    LOG.error("Error processing file {}: {}", selectedFile, e.getMessage(), e);
                    return Optional.empty();
                }
            }
        };
        Consumer<Runnable> finishTask = action -> Platform.runLater(() -> {
            toaster.removeToast();
            action.run();
            spinner.showSpinner(false);
        });
        task.setOnSucceeded(e -> finishTask.accept(() -> {
            Optional<T> result = task.getValue();
            result.ifPresentOrElse(
                    onSuccess::accept,
                    () -> toaster.addToast(
                            I18n.INSTANCE.getValue("toast.error.optionsviewmodel.ontaskcomplete"),
                            "",
                            ToastLevels.ERROR,
                            true
                    )
        ...
        task.setOnFailed(e -> finishTask.accept(() -> {
